...
class Space {
	...
	*[Symbol.iterator]() {
	...
	getField() {return new Field(this)};
...
class Field {
	constructor(
		...
		this.points = this.space.points.map(point => Point.clone(point))
	...
	static clone(self) {
		...
		clone.points = clone.points.map(point => Point.clone(point));
		clone.transformations = clone.transformations.slice(0);
		// TODO: add deep clone support for arrays, possibly second-level object literals
		clone.data = Object.assign({}, self.data);
		return clone;
	}
	
	static getTest(self) {
		return self.space.numPoints + '|' + Math.random().toString();
	}
	...
	*[Symbol.iterator]() {
	...
	get test() {
		return this._test_ = this._test_ || Field.getTest(this);
	}
	
	// calls transform method on clone of this, returns transformed clone
	transformMap(transformations, progress) {
		return Field.clone(this).transform(transformations, progress);
	}
	...
	transform(transformations, progress) {
		...
		for (let i = 0; i < this.space.numPoints; i++) {
			const point = this.points[i];
			point.position = this.space.positionsCartesian[i];
			point.transform(transReverse, progress);
		}
		return this;
	}
	...
	getCurveMesh({hideOuterCurves = false} = {}) {
		...
		const dimensionless = this.space.size.map(numPoints => numPoints === 1);
		// TODO: add methods to retrieve curve properties from points and vice versa
		// array of component-based multiplier arrays used to place points into appropriate curve sets
		const curveMultipliers = this.space.size.map((_, componentIndex, size) => {
			//////////////////
			if (dimensionless[componentIndex]) {return null};
		...
		// array of curve sets for each dimension (x-curveSet, y-curveSet, z-curveSet, ...)
		let curveMesh = this.space.size.map((numPoints, dimIndex, size) => {
			// array of curves for each curve set (x-curve_0, x-curve_1, ...)
			return [...Array(this.space.componentCurves[dimIndex])].map((_, curveIndex, curveArr) => {
				const multipliers = curveMultipliers[dimIndex];
				let constantCoordinateComponents = {};
				for (let i = 0; i < multipliers.length; i++) {
					// component where multiplier === 0 is curve's variable component
		...
		if (options.hideOuterCurves) {
			curveMesh = curveMesh.map((curveSet) => {
				return curveSet.filter((curve) => {
					let isInnerCurve = true;
					for (let [key, value] of Object.entries(curve.data.constantCoordinateComponents)) {
						if ((value === 0 || value === this.space.size[key] - 1) && !dimensionless[key]) {
							isInnerCurve = false;
							break;
						}
					}
...
const transWavy = new Transformation((r,theta) => [
...
let coords = new Coordinates([1,n_y,n_z], [
	150,
	[y_i, y_f],
	[z_i, z_f]
]);
let space = new Space([4,n_y,n_z], [
	[0,300],
	[y_i, y_f],
	[z_i, z_f]
]);

let coords2D = new Coordinates([n_y,n_z], [
	[y_i, y_f],
	[z_i, z_f]
]);
// coords2D.transform(transRadial).transform(transWavy);
coords2D.transform([transRadial,transWavy]);
// console.log(coords2D.points);
let point = new Point([-150,0]);
let transPoint = new Transformation((x,y,k) => [k*x + k*300, 50*Math.sin(k*2*Math.PI)]);
// let transPoint = new Transformation((x,y) => [2*x, y*y]);
// let pointT = point.transformMap([transPoint],.5);

// console.log(space);
let field = new Field(space);
let field2 = new Field(space);
// console.log(field);
// console.log(field.test);
// console.log(field.test);
// console.log(field2);
// console.log(field2.test);
// console.log(field2.test);
...
// 		{progress: 0, transformations: []},
// 		// {progress: 50, transformations: [wavy]},
// 		{progress: 100, transformations: [transSpherical]}
// 	]
// );
let animation = new CoordinatesAnimation(numFrames, coords2D, [
		{progress: 0},
		{progress: 100, transformations: [transRadial]}
	]
);
...
// let animationCurveSet = animation.frames.map(coords => coords.getCurveMesh({"hideOuterCurves": false}));
let mesh = coords2D.getCurveMesh({"hideOuterCurves": true});
